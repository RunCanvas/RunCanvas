...
/// 홈 상단 카드. 예전엔 아바타와 닉네임만 있어 오른쪽이 통째로 비어 있었다 —
/// 러너가 홈에서 한눈에 알고 싶은 것(내가 누구인지 · 어디까지 왔는지 · 이번 주 얼마나 남았는지)을 담는다.
/// 색은 쓰지 않는다: 레벨 컬러는 주요 버튼(러닝 시작)에만 쓴다는 앱 규칙 그대로.
struct ProfileHeader: View {
            ...
            NavigationLink {
                ...
                HStack(spacing: 12) {
                    ...
                    VStack(alignment: .leading, spacing: 2) {
                        ...
                        Text("\(userNickname.isEmpty ? "러너" : userNickname) 님 · \(level.title) 레벨")
                            .font(.headline)
                            .lineLimit(1)
                    }
    ...
    /// 왜 직접 그리는가: 기본 ProgressView 의 트랙이 Color.card(회색 8%) 위에서 거의 안 보인다.
    /// 색은 안 쓰고 불투명도만 올려 대비를 확보한다.
            ...
            ZStack(alignment: .leading) {
                ...
                Capsule()
                    ...
                    .frame(width: weekProgress > 0
                           ? max(geometry.size.width * weekProgress, 6)
                           : 0)
            }
        }
        .frame(height: 6)
